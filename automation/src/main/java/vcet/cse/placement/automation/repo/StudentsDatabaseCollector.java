...
public interface StudentsDatabaseCollector extends JpaRepository<Students, Long> {
    ...
    @Query("SELECT NEW map(s.name as name, s.universityNo as universityNo, " +
           "s.aptiTest1Score as test1, s.aptiTest2Score as test2, s.aptiTest3Score as test3) " +
           "FROM Students s " +
           "ORDER BY (s.aptiTest1Score + s.aptiTest2Score + s.aptiTest3Score) DESC")
    List<Map<String, Object>> findAllAptitudeScores();

    @Query("SELECT NEW map(s.name as name, s.universityNo as universityNo, " +
           "s.easyProblemsSolved as easy, s.mediumProblemsSolved as medium, " +
           "s.hardProblemsSolved as hard, s.totalProblemsSolved as total, " +
           "s.leetcodeScore as score) " +
           "FROM Students s " +
           "WHERE s.universityNo IS NOT NULL " +
